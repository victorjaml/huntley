 ...
 * becomes 1.
 *
 * Why patch `dns.lookup` rather than configure the HTTP client: huntley
 ...
 * so importing this file once (`http.mjs` does) covers every provider and
 ...
 *   - `HUNTLEY_NO_DNS_CACHE=1` opts out entirely — no memoization AND no
 *     pacing, since both live inside this patched lookup.
 *   - `HUNTLEY_DNS_LOOKUPS_PER_MIN` caps resolver-bound lookups
 *     (default 1200; `0` disables pacing but keeps the cache).
 *     Raised above the old per-subprocess 400 because every board lane,
 *     enrichment and fund resolve now share one process-wide limiter.
 */

import dns from 'node:dns';
import { inProviderFetch, isBlockedAddress, blockedAddressError } from './ip-guard.mjs';
...
const DEFAULT_LOOKUPS_PER_MIN = 1200;
...
export function lookupsPerMinFromEnv(env = process.env) {
  const raw = env.HUNTLEY_DNS_LOOKUPS_PER_MIN;
  ...
  if (!Number.isFinite(n) || n < 0) {
    console.error(
      `⚠ HUNTLEY_DNS_LOOKUPS_PER_MIN=${raw} is not a non-negative number — `
...
if (process.env.HUNTLEY_NO_DNS_CACHE !== '1') {
