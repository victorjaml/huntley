...
import './dns-cache.mjs'; // memoize dns.lookup process-wide (see that file)
import {
  DEFAULT_USER_AGENT,
  BROWSER_LIKE_USER_AGENT,
  MACOS_BROWSER_LIKE_USER_AGENT,
} from './user-agent.mjs';
import { providerFetchContext } from './ip-guard.mjs';
...
/** Combine an optional external AbortSignal with a local timeout controller. */
function linkSignals(external, local) {
  if (!external) return { signal: local.signal, unlink() {} };
  if (external.aborted) {
    local.abort();
    return { signal: local.signal, unlink() {} };
  }
...
async function fetchInContext(url, {
  ...
  // Honour a shared collection deadline by shrinking the per-request timeout.
  let effectiveTimeout = timeoutMs;
  if (deadlineAt != null) {
    const remaining = deadlineAt - Date.now();
    if (remaining <= 0) {
      const err = new Error('Aborted: collection deadline reached');
      err.name = 'AbortError';
      throw err;
    }
  ...
  const linked = linkSignals(externalSignal, controller);
...
export function abortError(message = 'Aborted: collection deadline reached') {
  const err = new Error(message);
  err.name = 'AbortError';
  return err;
}

/** Throw if the shared collection signal/deadline has already fired. */
export function assertNotCancelled(ctx = {}, opts = {}) {
  const signal = opts.signal ?? ctx?.signal ?? null;
  const deadlineAt = opts.deadlineAt ?? ctx?.deadlineAt ?? null;
  if (signal?.aborted) throw abortError('Aborted');
  if (deadlineAt != null && Date.now() >= deadlineAt) {
    throw abortError('Aborted: collection deadline reached');
  }
}

/**
 * Sleep that cancels when ctx.signal aborts or ctx.deadlineAt elapses.
 * Providers' pagination delays and retry backoff both route through this.
 */
export function sleep(ms, ctx) {
  ...
  if (deadlineAt != null) {
    const remaining = deadlineAt - Date.now();
    if (remaining <= 0) throw abortError('Aborted: collection deadline reached');
  ...
  return new Promise((resolve, reject) => {
    ...
    if (deadlineAt != null) {
      deadlineTimer = setTimeout(
        () => finish(reject, abortError('Aborted: collection deadline reached')),
...
export function isRetryableError(err) {
  ...
  if (/deadline|Aborted:/i.test(String(err?.message ?? ''))) return false;
...
async function withRetry(request, ctx, policy = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      let delayMs = retryAfterMs !== null
        ? Math.min(retryAfterMs, maxDelayMs * 4)
        : backoff + Math.random() * jitterMs;
      if (ctx?.deadlineAt != null) {
        delayMs = Math.min(delayMs, Math.max(0, ctx.deadlineAt - Date.now()));
      }
...
/**
 * Build a provider HTTP context. When `signal` / `deadlineAt` are set, every
 * request inherits them so a collection deadline cancels in-flight work.
 * `sleep` is likewise abortable so retry/pagination delays cannot overrun.
 *
 * @param {{signal?: AbortSignal|null, deadlineAt?: number|null}} [opts]
 */
export function makeHttpCtx({ signal = null, deadlineAt = null } = {}) {
  const ctx = {
    transport: 'http',
    signal,
    deadlineAt,
  };
  const withBudget = (fn) => (url, opts = {}) => fn(url, {
    ...opts,
    signal: opts.signal ?? signal,
    deadlineAt: opts.deadlineAt ?? deadlineAt,
  });
  ctx.sleep = (ms) => sleep(ms, { signal, deadlineAt });
  ctx.fetchJson = withBudget(fetchJson);
  ctx.fetchText = withBudget(fetchText);
  ctx.fetchResponse = withBudget(fetchResponse);
  ctx.fetchTextHead = withBudget(fetchTextHead);
  return ctx;
}
