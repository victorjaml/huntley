...
import { htmlToText } from '../http/html-to-text.mjs';
...
// to before this field list was widened. Uses the shared htmlToText pipeline
// (decode after strip) so entity-escaped tags cannot leak into the model text.
/** @param {any} posting */
function buildJobDescriptionText(posting) {
  const intro = htmlToText(posting?.jobPostSectionHtml?.introHtml, { mode: 'description' });
  const body = htmlToText(posting?.descriptionHtml, { mode: 'description' });
  const outro = htmlToText(posting?.jobPostSectionHtml?.outroHtml, { mode: 'description' });
  const compensation = htmlToText(posting?.compensationHtml, { mode: 'description' });
